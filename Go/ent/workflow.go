...
type WorkflowEdges struct {
	// ExecutionResults holds the value of the execution_results edge.
	ExecutionResults []*WorkflowExecutionResult `json:"execution_results,omitempty"`
	// loadedTypes holds the information for reporting if a
	// type was loaded (or requested) in eager-loading or not.
	loadedTypes [1]bool
}

// ExecutionResultsOrErr returns the ExecutionResults value or an error if the edge
// was not loaded in eager-loading.
func (e WorkflowEdges) ExecutionResultsOrErr() ([]*WorkflowExecutionResult, error) {
	if e.loadedTypes[0] {
		return e.ExecutionResults, nil
	}
	return nil, &NotLoadedError{edge: "execution_results"}
}
...
// QueryExecutionResults queries the "execution_results" edge of the Workflow entity.
func (w *Workflow) QueryExecutionResults() *WorkflowExecutionResultQuery {
	return NewWorkflowClient(w.config).QueryExecutionResults(w)
}
