...
type WorkflowQuery struct {
	...
	withExecutionResults *WorkflowExecutionResultQuery
...
// QueryExecutionResults chains the current query on the "execution_results" edge.
func (wq *WorkflowQuery) QueryExecutionResults() *WorkflowExecutionResultQuery {
	query := (&WorkflowExecutionResultClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withExecutionResults: wq.withExecutionResults.Clone(),
...
// WithExecutionResults tells the query-builder to eager-load the nodes that are connected to
// the "execution_results" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WorkflowQuery) WithExecutionResults(opts ...func(*WorkflowExecutionResultQuery)) *WorkflowQuery {
	query := (&WorkflowExecutionResultClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withExecutionResults = query
	return wq
}
...
//		GroupBy(workflow.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Workflow.Query().
//		Select(workflow.FieldCreateTime).
//		Scan(ctx, &v)
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := wq.withExecutionResults; query != nil {
		if err := wq.loadExecutionResults(ctx, query, nodes,
			func(n *Workflow) { n.Edges.ExecutionResults = []*WorkflowExecutionResult{} },
			func(n *Workflow, e *WorkflowExecutionResult) {
				n.Edges.ExecutionResults = append(n.Edges.ExecutionResults, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (wq *WorkflowQuery) loadExecutionResults(ctx context.Context, query *WorkflowExecutionResultQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *WorkflowExecutionResult)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workflowexecutionresult.FieldWorkflowID)
	}
	query.Where(predicate.WorkflowExecutionResult(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.ExecutionResultsColumn), fks...))
	}))
